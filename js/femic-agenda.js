...
async function renderInbox(){
  ...
  try {
    ...
    listEl.innerHTML = rows.map(r => {
      ...
      const phoneFmt = formatPatientPhone(r.phone);
      ...
      return '<div class="inbox-row" style="border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:8px;display:flex;gap:12px;align-items:flex-start" data-id="'+r.id+'">' +
        ...
        '<div style="display:flex;gap:4px;flex-shrink:0;flex-wrap:wrap">' +
          (r.status !== 'lida' ? '<button class="btn" style="font-size:0.75em" onclick="inboxMarkStatus(\''+escAttr(r.id)+'\',\'lida\')">Marcar lida</button>' : '') +
          (r.status !== 'resolvida' ? '<button class="btn primary" style="font-size:0.75em" onclick="inboxMarkStatus(\''+escAttr(r.id)+'\',\'resolvida\')">Resolver</button>' : '') +
          (r.jid ? '<button class="btn" style="font-size:0.7em;color:#ef4444;border-color:#ef4444" onclick="mutePatient(\''+escAttr(r.jid)+'\',30)">🔇 30min</button>' : '') +
          (r.jid ? '<button class="btn" style="font-size:0.7em;color:#ef4444;border-color:#ef4444" onclick="mutePatient(\''+escAttr(r.jid)+'\',120)">🔇 2h</button>' : '') +
        '</div>' +
      '</div>';
    }).join('');
  } catch(e) {
...
async function renderAppointmentSearch(){
  ...
  target.innerHTML=results.length?results.map(a=>{
    ...
    return `<div class="appointment-search-row status-${a.status}"><label class="appt-check"><input type="checkbox" ${checked} onchange="toggleAppointmentSearchSelection('${escAttr(a.id)}',this.checked)"></label><div class="appt-search-main"><strong>${esc(patient.name||'Paciente')}</strong><span>${fmtWeekday(a.appointment_date)} · ${fmtDate(a.appointment_date)} · ${normalizeTime(a.start_time)}-${normalizeTime(a.end_time)} · ${esc(service.name||'Sem serviço')}</span></div><span class="status-chip ${a.status}">${label}</span><div class="appt-search-actions"><button class="btn small" onclick="openAppt('${escAttr(a.appointment_date)}','${escAttr(a.id)}')">Editar</button><button class="btn small warning" onclick="cancelAppointmentFromSearch('${escAttr(a.id)}')">Cancelar</button><button class="btn small danger" onclick="deleteAppointmentFromSearch('${escAttr(a.id)}')">Apagar</button></div></div>`;
